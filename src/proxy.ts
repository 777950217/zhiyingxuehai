...
// ─── Route restrictions for financial role ───
const PUBLIC_ROUTES = [
  '/login', '/register', '/forgot-password', '/change-password',
  '/contact', '/intro', '/terms', '/privacy', '/robots.txt', '/favicon.ico',
];

const FINANCIAL_ONLY_ROUTES = [
  '/finance', '/finance/daily-profit', '/finance/accounts-receivable',
  '/finance/accounts-payable', '/finance/refund-compensation', '/finance/advertising',
  '/finance/cost-control', '/finance/warehouse', '/finance/monthly-close',
  '/finance/financial-analysis', '/finance/data-import',
];
...
export function proxy(request: NextRequest) {
  ...
  // Skip static assets and public routes for financial role check
  ...
  // Financial role route restriction - server-side check
  if (!PUBLIC_ROUTES.some(r => pathname.startsWith(r))) {
    try {
      const sessionCookie = request.cookies.get('sb-access-token');
      if (sessionCookie) {
        // Note: We can't make async calls in proxy, so we'll let client-side handle this
        // The middleware pattern has limitations in Next.js 16 proxy
      }
    } catch {
      // Ignore errors, let client-side handle auth
    }
  }
