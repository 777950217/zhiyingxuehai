...
export default function CertificateCard({ certificate, onClose }: CertificateCardProps) {
  ...
  const drawCertificate = useCallback((canvas: HTMLCanvasElement, includeQR = false) => {
    ...
    const line1 = `于 ${completedDate} 完成职盈学海客服主管管理能力`;
    ...
    ctx.fillText("课程阶段：角色认知 · 目标管理 · 团队带教 · 业务落地", textX, textY);
    ...
    ctx.textAlign = "left";
    ctx.fillText("颁发机构：职盈学海", textX, bottomY);
      ...
      <div className="bg-gradient-to-b from-amber-50 to-white border-2 border-amber-200 rounded-2xl p-8 relative overflow-hidden">
        ...
        <div className="text-center mb-6 px-4">
          <p className="text-lg text-gray-700 leading-relaxed">
            兹证明 <span className="font-bold text-blue-900 text-xl">{certificate.user_name}</span> 同学
          </p>
          <p className="text-base text-gray-600 mt-2">
            于 {completedDate} 完成职盈学海客服主管管理能力全部课程学习（共4阶段25节），考核通过，特发此证。
          </p>
          <p className="text-sm text-gray-400 mt-3">课程阶段：角色认知 · 目标管理 · 团队带教 · 业务落地</p>
        </div>

        <div className="flex items-end justify-between px-4 mt-6">
          <div className="text-left">
            <p className="text-sm font-bold text-blue-900">颁发机构：职盈学海</p>
