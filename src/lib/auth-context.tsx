...
export type UserRole = 'super_admin' | 'admin' | 'enterprise_admin' | 'enterprise_manager' | 'staff' | 'personal_user' | 'financial';

export type CompanyPlan = 'basic' | 'professional' | 'enterprise' | 'financial';
...
export interface UserProfile {
  ...
  /** Company's subscription plan: professional=专业版, enterprise=旗舰版, basic=个人版, financial=财务版 */
 ...
 * 三版本角色体系:
 *   admin           = 超级管理员, 全部功能
 *   enterprise_admin = 旗舰版老板, 全部功能含顾问后台
 *   enterprise_manager = 专业版主管/旗舰版班组长, 管理类全开(受plan-limits数量限制)
 *   staff            = 企业客服, 工作台+AI+培训+知识+工单(只读自己)
 *   personal_user    = 个人版, 学习/AI/知识类功能
 */
const FEATURE_ACCESS: Record<UserRole, string[]> = {
  super_admin: [
    ...
    'kpi-assessment', 'after-sales-guide', 'after-sales-product-videos', 'quality-feedback', 'my-knowledge',
  ],
  admin: [
    // 管理员：全功能访问
    'dashboard', 'my-workspace', 'companies', 'users', 'agents', 'teams', 'ai-assistant',
  ...
  enterprise_admin: [
    // 旗舰版老板：全业务功能 + 财务看板 + 管理方案（顾问后台仅服务商角色可见）
    'dashboard', 'my-workspace', 'companies', 'users', 'agents', 'teams', 'ai-assistant',
    'product-knowledge', 'work-orders', 'customer-records', 'training',
    ...
    'after-sales-guide', 'after-sales-product-videos', 'quality-feedback', 'my-knowledge',
    'kpi-assessment',
    'team-members',
    // 财务功能
    'daily-profit', 'accounts-receivable', 'accounts-payable', 'refund-compensation',
    'advertising', 'cost-control', 'warehouse', 'monthly-close', 'financial-analysis', 'data-import',
  ],
  enterprise_manager: [
    // 旗舰版主管：全业务功能 + 审批 + 财务只读+导入
    ...
    'advertising', 'cost-control', 'warehouse', 'monthly-close', 'financial-analysis', 'data-import',
    // 管控看板和驾驶舱
    'cockpit', 'cost-baseline', 'profit-funnel', 'ai-checkup',
  ],
  staff: [
    // 客服：个人工作台 + 学习
  ...
  personal_user: [
    // 个人版：课程学习 + AI助手 + 自我提升 + KPI学习/对话自检/经营周览/月度简报
    ...
    'kpi-assessment', 'after-sales-guide', 'after-sales-product-videos', 'quality-feedback', 'my-knowledge',
    // 新增开放功能
    'kpi', 'self-check', 'boss-weekly', 'monthly-report', 'newbie-training',
  ],
  financial: [
    // 财务版：财务相关功能
    'dashboard', 'help', 'settings', 'contact',
...
const ROUTE_FEATURE_MAP: Record<string, string> = {
  ...
  '/after-sales/product-videos': 'after-sales-product-videos',
...
export function getRoleLabel(role: UserRole): string {
  switch (role) {
    ...
    case 'enterprise_admin': return '专业版老板';
    case 'enterprise_manager': return '主管/班组长';
    case 'personal_user': return '个人版用户';
    case 'financial': return '财务版用户';
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const loadProfile = useCallback(async (sb: SupabaseClient, userId: string, forceRefresh = false) => {
    ...
    try {
      ...
      const res = await fetch(`/api/auth/profile?user_id=${userId}`, { headers });
      if (res.ok) {
        const data = await res.json();
        const newProfile = data.profile;

        // Check account status (disabled users cannot login)
if (newProfile?.status === 'disabled') {
   ...
   *  personal_user / efficiency_user are paying customers, NOT enterprise staff. */
  const isEnterpriseStaff = useCallback((): boolean => {
    return profile?.role === 'staff' || profile?.role === 'enterprise_manager';
  ...
  const contextValue = React.useMemo(() => ({
    ...
    authFetch,
    setProfile,
  }), [user, session, profile, loading, accountExpired, signOut, refreshProfile, supabase, hasAccess, isEnterpriseStaff, authFetch, setProfile]);
