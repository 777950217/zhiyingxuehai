...
const VALID_TYPES = ['script', 'sop', 'case', 'quality', 'plan'] as const;
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (subErr) {
      console.error('[ai-checkup/submit] insert submission error:', subErr.message);
    ...
    if (phraseErr) {
      console.error('[ai-checkup/submit] insert phrase error:', phraseErr.message);
    ...
    console.error('[ai-checkup/submit] error:', message);
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (error) {
      console.error('[ai-checkup/submit] GET error:', error.message);
    ...
    const message = err instanceof Error ? err.message : '未知错误';
    console.error('[ai-checkup/submit] GET error:', message);
    return NextResponse.json({ error: '查询失败' }, { status: 500 });
  }
}
