...
const DEFAULT_SHIFTS: ShiftItem[] = [
  ...
  { label: '通宵�?(0:00-8:00)', staff: '', hours: '8' },
];

const DEFAULT_EMERGENCY: EmergencyPlan[] = [
  {
    trigger: '流量突增超过预估2�?,
    steps: ['启动备用客服账号，临时增开接待�?, '调整自动回复，引导客户自助查询常见问�?, '紧急调配其他岗位人员临时支�?],
    escalation: '持续1小时未缓解→通知主管，启动外部临时工方案',
  },
  {
    trigger: '客诉量暴增，差评率超�?%',
    steps: ['集中处理高优先级投诉（退�?换货�?, '统一口径回复，避免不同客服说法不一', '记录典型客诉案例，活动后复盘'],
    escalation: '单日差评率超�?0%→通知老板，启动专项赔偿方�?,
  },
  {
    trigger: '系统/平台故障，无法正常接�?,
    steps: ['立即切换到备用联系方式（电话/微信�?, '在店铺首页公告故障情�?预计恢复时间', '故障恢复后优先处理积压订�?],
    escalation: '故障超过30分钟→通知老板，考虑活动延期或补偿方�?,
  },
];
...
const PRESET_SCRIPTS: Record<string, Record<string, string>> = {
  rulesScript: {
    ...
    '预售': '亲，预售商品需先付定金XX元，尾款在XX月XX�?XX日支付，逾期定金不退哦~付尾款后XX天内发货~',
...
export default function TabPromotionPlan({ userId }: { userId: string }) {
    ...
    try {
      const res = await fetch('/api/personal/reports', {
        ...
        body: JSON.stringify({
          ...
          template: 'promo_plan',
          metrics: {
            activityName: promoPlan.name || '未命名活�?,
  ...
  const exportPromoPlan = () => {
    const lines: string[] = [
      ...
      `- 临时�?借调�?{promoPlan.tempStaff || '�?}`,
      '',
      '### 排班方案',
      ...promoPlan.shifts.map(s => `- ${s.label}�?{s.staff || '待分�?}�?{s.hours}小时）`),
      '',
      '## 话术预案',
      `### 活动规则话术\n${promoPlan.rulesScript || '未填�?}`,
      `### 库存不足话术\n${promoPlan.stockOutScript || '未填�?}`,
      `### 发货延迟话术\n${promoPlan.delayShipScript || '未填�?}`,
      `### 退�?售后话术\n${promoPlan.refundScript || '未填�?}`,
      '',
      '## 应急预�?,
    ...
    a.href = url;
    a.download = `大促预案-${promoPlan.name || '未命�?}-${new Date().toISOString().slice(0, 10)}.md`;
    a.click();
    ...
    <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500 mt-1">
              按{promoPlan.volumeMultiplier}倍流量，建议增配 {Math.max(1, Math.round(promoPlan.teamSize * (parseFloat(promoPlan.volumeMultiplier) - 1) * 0.5))} �?
            </p>
          </div>
          <div>
            <FieldLabel>每人每日最长工�?/FieldLabel>
      ...
      <Section id="shifts" icon={<Clock className="w-5 h-5" />} title="大促排班方案" color="bg-orange-50">
        <div className="space-y-3">
          ...
          <button
            onClick={() => updatePlan('shifts', [...promoPlan.shifts, { label: '新班�?, staff: '', hours: '8' }])}
      ...
      {/* 应急预�?*/}
      <Section id="emergency" icon={<AlertTriangle className="w-5 h-5" />} title="应急预�? color="bg-red-50">
        <div className="space-y-4">
      ...
      {promoResult && (
        <div className="p-4 bg-gradient-to-br from-orange-50 to-amber-50 border border-orange-200 rounded-xl">
          <h3 className="text-lg font-bold text-orange-800 mb-2 flex items-center gap-2">
            <Sparkles className="w-5 h-5" /> AI生成的大促预案建�?
          </h3>
      ...
      <Section id="history" icon={<FileText className="w-5 h-5" />} title="历史预案" color="bg-gray-50">
        {promoHistory.length === 0 ? (
          <p className="text-gray-500 text-center py-4">暂无历史预案，生成后会自动保�?/p>
        ) : (
          <div className="space-y-2">
