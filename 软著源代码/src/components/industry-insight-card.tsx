...
export function IndustryInsightCard({ topic, context }: IndustryInsightCardProps) {
  ...
  // 如果已完成档案且有profile_data，自动生成行业视�?
  useEffect(() => {
    if (hasProfile && industryData && !insight && !loading) {
...
用户行业�?{industryName}，主营产品：${mainProduct}${materialSuffix}，团队规模：${teamSize}�?
行业档案摘要�?
...
请用100-200字，说明�?{topic}」这个方法在${industryName}行业里具体怎么用�?{materialInfo ? `结合${materialInfo}材质特点。` : ''}要结合行业特点给出实操建议，不要泛泛而谈。`,
          systemPrompt: `你是一�?{industryName}行业的资深客服管理顾问。请用通俗简洁的语言，结合用户行业特点，说明课程方法在该行业的具体应用。直接输出内容，不要加标题。`,
        }),
      });
  ...
  return (
    <Card className="mt-6 border-blue-100 bg-gradient-to-br from-blue-50 to-sky-50">
      ...
      <CardContent>
        {loading ? (
        ...
        ) : industryData && !industryData.complaint_types ? (
          <div className="py-3">
          ...
          <div className="flex items-center gap-3 py-4">
            <Loader2 className="w-5 h-5 text-blue-600 animate-spin" />
            <span className="text-base text-gray-600">正在加载...</span>
          </div>
        )}
      </CardContent>
    </Card>
