...
export function OnboardingGuideCard() {
  ...
  const guideConfigs: Record<string, { key: string; steps: GuideStep[] }> = {
    '/kpi': {
      key: 'kpi',
      steps: [
        { title: '设置4个核心指�?, description: '选择响应时长、满意度、转化率、赔付率作为核心考核指标' },
    ...
    '/teams': {
      ...
      steps: [
        { title: '创建班次（早/�?晚）', description: '根据业务量设置不同时段的班次安排' },
        { title: '给员工排�?, description: '将团队成员分配到对应班次' },
    ...
    '/business-tools': {
      ...
      steps: [
        { title: '录入团队月固定成�?, description: '填写客服人均月成本等固定支出' },
        { title: '录入退�?赔付均单成本', description: '填写退货运费和售后赔付的平均金�? },
    ...
    '/approval': {
      key: 'approval',
      steps: [
        { title: '设置主管审批阈�?, description: '建议设置300-500元，低于此金额主管可直接审批' },
        { title: '设置老板审批阈�?, description: '建议设置1000-2000元，超过此金额需要老板审批+备注' },
        { title: '保存后审批规则自动生�?, description: '配置完成后，所有赔付记录将按规则自动走审批流程' },
      ],
    },
  };
